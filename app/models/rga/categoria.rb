class Rga::Categoria < ActiveRecord::Base
  ...
  def self.categorias_disponibles(empresa_id, establecimiento_id, options = {})
  	...
  	estados = options[:estados] || '1'
		categorias = Rga::Categoria.find_by_sql("select e.* from (SELECT a.*, max(c.id) as 
		registro_id from rga_animales as a join rga_animales_registros as b 
		on a.id = b.animal_id join rga_registros as c on b.registro_id = c.id where a.estado in (#{estados}) and c.fecha <= '#{fecha}' group by a.id) as sc 
		join rga_registros as d on sc.registro_id = d.id
		join rga_categorias as e on d.destcategoria_id = e.id
		where d.empresa_id = #{empresa_id} and d.establecimiento_id = #{establecimiento_id} group by e.id ")
	end
