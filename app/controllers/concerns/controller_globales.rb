module ControllerGlobales
	...
	def arbol_index(nodo, args = {})
		...
		if nodo
			case nodo
			 when 'raiz'
				instance_variable_set(v[:coleccion], Rba::Arbol.find_by_modelo(controller_path).
					nodos.first.send(v[:metodo]).where(contexto).order(args[:order]))
				when Rba::Arbol.find_by_modelo(controller_path).nodos.first.id.to_s
					instance_variable_set(v[:coleccion], v[:clase].where(contexto).includes(includes).order(args[:order]))
				else 
					instance_variable_set(v[:coleccion], Rba::Nodo.find(nodo).send(v[:metodo]).where(contexto).order(args[:order]))
			end
		else  
